package com.example.ayuan.utils;
    ...
    public long judgeLimit(String key, int limit) {
        ...
        return script.eval(RScript.Mode.READ_WRITE,
                "local count = redis.call('get', KEYS[1]);" +
                        "if count then " +
                        "if count>=ARGV[1] then count=-1 else redis.call('incr',KEYS[1]) end " +
                        "else count = 1 redis.call('set', KEYS[1],count) end;" +
    ...
    public long returnCount(String key) {
        ...
        return script.eval(RScript.Mode.READ_WRITE,
                "local count = tonumber(redis.call('get', KEYS[1]));" +
                        "if count then " +
                        "if count>0 then count=count-1 redis.call('set', KEYS[1],count) redis.call('expire',KEYS[1],ARGV[1]) else count = 0 end " +
                        "else count = 0 end;" +
                        "return count;",
    ...
    public String generateOrder(String keyPrefix) {
        ...
        Long eval = script.eval(RScript.Mode.READ_WRITE,
                "local sequence = redis.call('get', KEYS[1]);" +
                        "if sequence then if sequence>ARGV[1] then sequence = 0 else sequence = sequence+1 end " +
                        "else sequence = 1 end;" +
